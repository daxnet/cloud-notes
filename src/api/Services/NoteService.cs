...
namespace CloudNotes.Api.Services
{
    public class NoteService : INoteService
    {
        ...
        {
            const string sql = "UPDATE notes SET \"Title\"=@Title, \"Content\"=@Content, \"DateModified\"=@DateModified WHERE \"Id\"=@id";
            await using var connection = new NpgsqlConnection(_connectionString);
            var rowsAffected = await connection.ExecuteAsync(sql, new { note.Title, note.Content, DateModified = DateTime.UtcNow, id });
            return rowsAffected > 0;
        }
